...
class DiaWatchModel(mlflow.pyfunc.PythonModel):
    """
     model wrapper handling imputation and thresholding.
    """
    ...
    def predict(self, context, model_input: pd.DataFrame):
        ...
        # Diabetes first (highest priority)
...
def load_and_prepare_data(data_path: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    required_cols = [
        'gender', 'race_ethnicity', 'is_pregnant', 'family_history',
        'age', 'bmi', 'waist_circumference', 'target'
    ]
...
def train_and_evaluate(
    ...
    class_names = {0: 'Healthy', 1: 'Prediabetes', 2: 'Diabetes'}
    ordered_weights = [
    ...
    with mlflow.start_run(run_name=f"DiaWatch_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        mlflow.log_params(params)
        mlflow.log_param("train_size", len(X_train))
        mlflow.log_param("test_size", len(X_test))
        
        model = CatBoostClassifier(**params)
        model.fit(X_train_imp, y_train, eval_set=(X_test_imp, y_test))
        ...
        reference_data = reference_data[['gender', 'race_ethnicity', 'is_pregnant', 
                                        'family_history', 'age', 'bmi', 'waist_circumference', 'target']]
        ...
        artifacts = {
            "catboost_model": temp_catboost_path,
        ...
        client.set_registered_model_alias("DiaWatch_Medical_System", "production", new_version)

        
        logger.info(f"Training complete. Macro Recall: {macro_recall:.3f}, Macro F1: {macro_f1:.3f}")
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(base_dir, 'data', 'clean', 'cleaned_data.parquet')
    export_dir = os.path.join(base_dir, 'exports', datetime.now().strftime("%Y%m%d_%H%M%S"))
