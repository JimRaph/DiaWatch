...
from fastapi import APIRouter, Depends, HTTPException, status,Request, Response, Cookie, Header, Body
...
from schema import UserCreate, TokenPair, TokenResponse, RefreshTokenRequest, UserResponse
...
def register_user(
    ...
    try:
        ...
        if guest_id:
            if user.link_guest:
                print(f"linking guest history {guest_id} to user {new_user.id} with {user.link_guest}")
                
                db.query(PredictionStore).filter(
                    PredictionStore.guest_id == guest_id).update(
                    {"user_id": new_user.id,
                "is_guest": False}, synchronize_session=False)

                db.query(MonitorStore).filter(
                    MonitorStore.guest_id == guest_id).update(
                    {"is_guest": False}, synchronize_session=False) 
