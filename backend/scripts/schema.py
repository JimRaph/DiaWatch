...
class ClinicalFeatures(BaseModel):
    ...
    waist_circumference: float = Field(None, gt=30, lt=200, description="Waist circumference in cm")
...
class DiaRequest(BaseModel):
    ...
    health_note: Optional[str] = Field(None, max_length=1000, description="Free text health concerns")


class FeedbackRequest(BaseModel):
    """Feedback submission schema."""
    prediction_id: str = Field(..., description="UUID from prediction response")
    actual_class: int = Field(..., ge=0, le=2, description="0=Healthy, 1=Prediabetes, 2=Diabetes")
...
class DiaWatchReport(BaseModel):
    ...
    risk_explanation: str = Field(description="Explanation of top 2 risk factors in plain language")
    recommendations: List[RecommendationItem] = Field(description="Exactly 3 actionable tips")
...
class PredictionResponse(BaseModel):
    ...
    timestamp: datetime
    verification_status: Optional[str] = None  # pending, verified, corrected, unverified
