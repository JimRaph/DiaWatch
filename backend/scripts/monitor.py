...
# Ensure you import your model and DB settings from where they live in your project
from database import DATABASE_URL
from database import MonitorStore  # <-- Update this import path to match your project
...
class DiaWatchMonitor:
    ...
    def __init__(self):
        ...
        self.model_name = os.getenv("MLFLOW_MODEL_NAME", "DiaWatch_Medical_System")
    ...
    def _prepare_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cat_cols = ["gender", "race_ethnicity", "is_pregnant", "family_history", "prediction", "target"]
        num_cols = ["age", "bmi", "waist_circumference"]

        for col in cat_cols:
            if col in df.columns:
                df[col] = df[col].astype(str).replace(['None', 'nan', 'NaN'], pd.NA).astype('category')
    ...
    def fetch_reference_data(self) -> pd.DataFrame:
        ...
        if not os.path.exists(reference_file):
            raise MonitoringError(f"Critical error: reference_data.parquet not found in MLflow artifacts.")
    ...
    def fetch_drift_data(self, days: int = 30) -> pd.DataFrame:
        ...
        with Session(self.engine) as session:
            query = session.query(
                MonitorStore.age, MonitorStore.bmi, MonitorStore.waist_circumference,
                MonitorStore.gender, MonitorStore.race_ethnicity, MonitorStore.is_pregnant,
                MonitorStore.family_history, MonitorStore.prediction
            ).filter(
    ...
    def fetch_performance_data(self, days: int = 60) -> pd.DataFrame:
        ...
        with Session(self.engine) as session:
            query = session.query(
                ...
                MonitorStore.actual.isnot(None),
                MonitorStore.timestamp >= cutoff_date
            )
            df = pd.read_sql(query.statement, session.bind)
            ...
            if not df.empty:
                ...
                df['feedback_delay'] = pd.to_datetime(df['feedback_timestamp']) - pd.to_datetime(df['timestamp'])
                df = df[df['feedback_delay'] <= pd.Timedelta(days=14)]
                df = df.drop(columns=['timestamp', 'feedback_timestamp', 'feedback_delay'])
                
            logger.info(f"Loaded {len(df)} samples with ground truth.")
            return self._prepare_df(df)

    def run_monitoring_suite(self, drift_days: int = 30, perf_days: int = 60) -> Dict[str, Any]:
        try:
            reference_df = self.fetch_reference_data()
            training_metrics = self.get_training_metrics()

            if "prediction" not in reference_df.columns and "target" in reference_df.columns:
            ...
            data_def = DataDefinition(
                ...
                categorical_columns=["gender", "race_ethnicity", "is_pregnant", "family_history"],
            ...
            if not drift_df.empty and len(drift_df) < self.min_samples_drift:
                logger.warning(f"Not enough data for drift analysis: {len(drift_df)} samples")
                ...
                if "target" in reference_df.columns:
                    drift_df["target"] = drift_df["target"].astype(
                        pd.CategoricalDtype(categories=reference_df["target"].cat.categories)
                    )
                ...
                metric_results = [m.get("result", {}) for m in report_dict.get("metrics", [])]
                drift_summary = next((m for m in metric_results if "share_of_drifted_columns" in m), {})
                ...
                results["drift"] = {
                    ...
                    "drifted_columns": drift_summary.get("number_of_drifted_columns", 0)
                }
            ...
            if not perf_df.empty and len(perf_df) < self.min_samples_performance:
                logger.warning(f"Not enough data for performance analysis: {len(perf_df)} samples")
            elif perf_df.empty:
                logger.warning("No data for performance analysis")
            elif len(perf_df) >= self.min_samples_performance:
                print("random log to check 4")
                ref_perf_ds = Dataset.from_pandas(reference_df.dropna(subset=['target']), data_definition=data_def)
                cur_perf_ds = Dataset.from_pandas(perf_df, data_definition=data_def)
                
                perf_report = Report(metrics=[ClassificationPreset()])
                perf_result = perf_report.run(reference_data=ref_perf_ds, current_data=cur_perf_ds)
                

                perf_dict = perf_result.dict()
                current_metrics = {}
                for m in perf_dict.get("metrics", []):
                    if "current" in m.get("result", {}) and "f1" in m["result"]["current"]:
                ...
                if (baseline_f1 - current_f1) > 0.10:
                    self._send_alert(results, f"Performance Drop! F1 degraded by {round((baseline_f1-current_f1)*100)}%")
    ...
    def _save_report(self, report: Report, analysis_type: str, days: int, metadata: Dict):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_dir = os.getenv("MONITORING_OUTPUT_DIR", "/tmp/monitoring")
        os.makedirs(output_dir, exist_ok=True)
        
        report_path = os.path.join(output_dir, f"report_{analysis_type}_{days}d_{timestamp}.html")
        json_path = os.path.join(output_dir, f"results_{analysis_type}_{days}d_{timestamp}.json")
