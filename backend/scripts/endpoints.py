...
from fastapi import APIRouter, Depends, BackgroundTasks, HTTPException, Request, Response, Cookie, status
from sqlalchemy.orm import Session
from sqlalchemy.exc import SQLAlchemyError
from sqlalchemy import func, or_, and_

from database import get_db, SessionLocal, PredictionStore, MonitorStore, log_prediction_for_monitoring, User
...
async def predict(
    ...
    """
    Diabetes screening prediction with personalized recommendations.
    """
    request_id = getattr(http_request.state, 'request_id', str(uuid7()))
    ...
    try:
        ...
        for feature, (min_val, max_val) in TRAINING_RANGES.items():
            if feature in clinical_data and clinical_data[feature] is not None:
                clinical_data[feature] = float(np.clip(clinical_data[feature], min_val, max_val))
        
        if clinical_data.get("gender") == "Male":
            clinical_data["is_pregnant"] = "No"
        
        pred_class, probabilities, explanations = inference_engine.predict(clinical_data)
        ...
        try:
            report = advisor.generate_report(
                ...
                lifestyle_data=request.lifestyle_features.dict() if request.lifestyle_features else {},
...
async def submit_feedback(
    ...
    """
    try:
        record = db.query(PredictionStore).filter(
            PredictionStore.id == request.prediction_id
        ...
        monitor_record = db.query(MonitorStore).filter(
            MonitorStore.prediction_id == request.prediction_id
        ).with_for_update().first()
...
async def decline_verification(
    ...
    """
    User explicitly declines to verify this prediction (e.g., won't see a doctor).
    Distinguishes 'I won't verify' from 'I haven't verified yet'.
    """
    try:
        record = db.query(PredictionStore).filter(
            PredictionStore.id == prediction_id
        ).first()
        ...
        if record.verification_status != 'pending':
            raise HTTPException(400, f"Cannot decline verification with status: {record.verification_status}")
...
def persist_prediction(
    ...
    for attempt in range(max_retries):
        try:
            ...
            db.commit()
            logger.info(f"Prediction {prediction_id} persisted successfully in background.")
            break
...
async def get_user_history(
    ...
    query = db.query(PredictionStore).filter(
        PredictionStore.user_id == current_user.id
    )
    ...
    records = query.order_by(
        PredictionStore.timestamp.desc(),
        PredictionStore.id.desc()  
    ).limit(limit + 1).all()
