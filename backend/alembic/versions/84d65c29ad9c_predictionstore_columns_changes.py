...
def upgrade() -> None:
    """Upgrade schema."""
    op.add_column(
        "predictions", sa.Column("verification_status", sa.String(), nullable=True)
    )

    # Create index for faster queries
    op.create_index(
        "ix_predictions_verification_status", "predictions", ["verification_status"]
    )
    ...
    op.alter_column(
        "predictions", "verification_status", server_default="pending", nullable=True
    )
