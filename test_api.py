...
PAYLOADS = {
    ...
    "feedback": {
        "prediction_id": "019d4b38-b445-71f4-8ace-62c8f910b64a",
        "actual_class": 0,
    },
}
    ...
    for i in range(count):
        ...
        payload = {
            ...
            "lifestyle_features": {
                ...
                "exercise_frequency": ["Never", "Rarely", "Few times a week", "Daily"][
                    i % 4
                ],
        ...
        if response.status_code == 200:
            ...
            print(
                f"[{i + 1}/{count}] ✓ Created: {pred_id} | Result: {data.get('clinical_prediction')}"
            )
        else:
            print(
                f"[{i + 1}/{count}] ✗ Failed: {response.status_code} - {response.text}"
            )
...
def submit_feedback_batch(
    feedback_data: List[Dict[str, any]], token: Optional[str] = None
):
    ...
    for i, item in enumerate(feedback_data, 1):
        ...
        if response.status_code == 200:
            print(
                f"[{i}/{len(feedback_data)}] ✓ Feedback for {item['prediction_id'][:20]}... | Class: {item['actual_class']}"
            )
            success_count += 1
        else:
            print(
                f"[{i}/{len(feedback_data)}] ✗ Failed: {response.status_code} - {response.text[:100]}"
            )
            failed.append(item)
    ...
    if failed:
        ...
        for item in failed:
            print(
                f'    {{"prediction_id": "{item["prediction_id"]}", "actual_class": {item["actual_class"]}}},'
            )
        print("]")
...
def main():
    ...
    parser.add_argument(
        "--populate", "-p", action="store_true", help="Populate 10 predictions"
    )
    parser.add_argument(
        "--count",
        "-c",
        type=int,
        default=10,
        help="Number of predictions to create (default: 10)",
    )
    parser.add_argument(
        "--feedback", "-f", action="store_true", help="Submit feedback for predictions"
    )
    ...
    if args.endpoint == "predict":
        ...
        if response.status_code == 200:
            ...
            print("Refresh Token stored in session cookie jar")
